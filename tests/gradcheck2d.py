...
for (dt, dtn, eps, atol, rtol, ndtol, fm) in [
    ...
    if gradcheck(NATTEN2DQKRPBFunction.apply, variables, eps=eps, atol=atol, rtol=rtol, nondet_tol=ndtol, fast_mode=fm):
    ...
    if gradcheck(NATTEN2DAVFunction.apply, variables, eps=eps, atol=atol, rtol=rtol, nondet_tol=0, fast_mode=fm):

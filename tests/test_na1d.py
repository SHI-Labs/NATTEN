...
def _priv_test_gradcheck_natten1dqk(
        ...
        dtype, device, eps, atol, rtol, ndtol,
        fast_mode):
    ...
    assert gradcheck(
            ...
            rtol=rtol,
            nondet_tol=ndtol,
            fast_mode=fast_mode), f"Gradcheck failed for `NATTEN1D-QK` with {dtype} on {device}, {md} MODE."
...
def _priv_test_gradcheck_natten1dqkrpb(
    ...
    assert gradcheck(
            ...
            rtol=rtol, 
            nondet_tol=ndtol, 
            fast_mode=fast_mode), f"Gradcheck failed for `NATTEN1D-QKRPB` with {dtype} on {device}, {md} MODE."
...
def _priv_test_gradcheck_natten1dav(
    ...
    assert gradcheck(
            ...
            rtol=rtol, 
            nondet_tol=ndtol, 
            fast_mode=fast_mode), f"Gradcheck failed for `NATTEN1D-AV` with {dtype} on {device}, {md} MODE."


def _priv_test_allclose_cpu_cuda(batch_size, length, 
        ...
        for dim in dims:
            for num_heads in heads:
                for rpb in [True, False]:
                    ...
                    base_state_dict = NeighborhoodAttention1D(**model_kwargs).state_dict()
                    ...
                    assert forward_mse < tol, \
                        f"FAIL: Forward MSE ({forward_mse}) was above the specified tolerance " \
                        f"(tol) for heads={heads}, dim={dim}, kernel_size={kernel_size}."
                    ...
                    for name, n1w in nat1.named_modules():
                        ...
                        for name2, n2w in nat2.named_modules():
                            ...
                            if n1w.weight.grad is None or n2w.weight.grad is None:
                                continue
                            mse = ((n1w.weight.grad - n2w.weight.grad.cpu())**2).mean()
                            if hasattr(n1w, 'bias') and n1w.bias is not None:
                                if hasattr(n1w.bias, 'grad') and hasattr(n2w.bias, 'grad'):
                                    mse += ((n1w.bias.grad - n2w.bias.grad.cpu())**2).mean()

                            assert mse < tol, \
                                f"FAIL: {name} gradient MSE ({mse}) was above the specified tolerance " \
                                f"({tol}) for heads={heads}, dim={dim}, kernel_size={kernel_size}."
...
class NA1DTest(unittest.TestCase):
    def test_natten1dqk_gradcheck_cpu_slow(self):
        ...
        _priv_test_gradcheck_natten1dqk(b, h, l, d, k, di, torch.float64, 'cpu',
                                     1e-6, 1e-5, 1e-3, 0, False)

    def test_natten1dqk_gradcheck_cpu_fast(self):
        b, h, l, d, k, di = 1, 2, 14, 32, 7, 2
        _priv_test_gradcheck_natten1dqk(b, h, l, d, k, di, torch.float64, 'cpu',
                                     1e-6, 1e-5, 1e-3, 0, True)

    def test_natten1dqk_gradcheck_cuda_slow(self):
        if not HAS_CUDA:
            self.skipTest("CUDA not available.")
        b, h, l, d, k, di = 1, 2, 14, 4, 7, 2
        _priv_test_gradcheck_natten1dqk(b, h, l, d, k, di, torch.float64, 'cuda',
    ...
    def test_natten1dqk_gradcheck_cuda_fast(self):
        ...
        b, h, l, d, k, di = 1, 2, 14, 32, 7, 2
        _priv_test_gradcheck_natten1dqk(b, h, l, d, k, di, torch.float64, 'cuda',
                                     1e-6, 1e-5, 1e-3, 1e-8, True)

    def test_natten1dqkrpb_gradcheck_cpu_slow(self):
        b, h, l, d, k, di = 1, 2, 14, 4, 7, 2
        _priv_test_gradcheck_natten1dqkrpb(b, h, l, d, k, di, torch.float64, 'cpu',
                                     1e-6, 1e-5, 1e-3, 0, False)
    ...
    def test_natten1dqkrpb_gradcheck_cuda_slow(self):
        if not HAS_CUDA:
            self.skipTest("CUDA not available.")
        b, h, l, d, k, di = 1, 2, 14, 4, 7, 2
        _priv_test_gradcheck_natten1dqkrpb(b, h, l, d, k, di, torch.float64, 'cuda',
                                     1e-6, 1e-5, 1e-3, 1e-8, False)
    ...
    def test_natten1dav_gradcheck_cpu_slow(self):
        b, h, l, d, k, di = 1, 2, 14, 4, 7, 2
        _priv_test_gradcheck_natten1dav(b, h, l, d, k, di, torch.float64, 'cpu',
                                     1e-6, 1e-5, 1e-3, 0, False)
    ...
    def test_natten1dav_gradcheck_cuda_slow(self):
        if not HAS_CUDA:
            self.skipTest("CUDA not available.")
        b, h, l, d, k, di = 1, 2, 14, 4, 7, 2
        _priv_test_gradcheck_natten1dav(b, h, l, d, k, di, torch.float64, 'cuda',
                                     1e-6, 1e-5, 1e-3, 1e-8, False)

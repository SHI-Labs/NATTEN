...
struct CollectiveMainloopFwdSm90 {
    ...
    static constexpr bool Use_TMA_KV = false; // !PagedKVNonTMA
                                              // (aditya) NOTE: PagedKVNonTMA is assumed to be true
    ...
    using BlockMN_t = flash::BlockMN<SeqlenInfo_t, kBlockM, kBlockN, PackGQA>; // removed split arg
    ...
    mma(Params const& params,
        ...
        if constexpr (IntraWGOverlap) {
            ...
            mask.template apply<true /*Seqlenk_mask*/>(tSrS, m_block, n_block);
    ...
    load_kv_new(Params const& params,
        ...
        bool const is_varlen_k_new = false;
    ...
    store_kv_new(Params const& params,
        ...
        static_assert(std::is_same_v<GmemLayoutAtom, typename PagedKVManager_t::GmemLayoutAtomKVCpAsync>);
        ...
        auto store_K = [&] (int const n_block, auto const& smem_pipe_read) {
            ...
            if (get<1>(params.shape_rotary) <= 0) {
                ...
                auto tPrKPtr = paged_kv_manager.compute_K_ptr();
                if (params.is_rotary_interleaved) {
                    auto [tRrCos, tRrSin] = rotary.template load_cos_sin<true /*kInterleaved*/>(n_block);
                    pipeline_k_new.consumer_wait(smem_pipe_read);
                    rotary.template apply_K_interleaved<true>(sK(_, _, smem_pipe_read.index()), gK_cur, tKpK, tRrCos, tRrSin, tPrKPtr, n_block);
                } else {
                    auto [tRrCosCont, tRrSinCont] = rotary.template load_cos_sin<false /*kInterleaved*/>(n_block);
                    pipeline_k_new.consumer_wait(smem_pipe_read);
                    rotary.template apply_K_contiguous<true>(sK(_, _, smem_pipe_read.index()), gK_cur, tKpK, tRrCosCont, tRrSinCont, tPrKPtr, n_block, get<1>(params.shape_K));
                }
            }

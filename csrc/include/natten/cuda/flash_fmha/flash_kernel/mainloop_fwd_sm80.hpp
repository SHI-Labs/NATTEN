...
struct CollectiveMainloopFwdSm80 {
    ...
    //             auto tPrKPtr = nullptr;
    //             if (params.is_rotary_interleaved) {
    //                 auto [tRrCos, tRrSin] = rotary.template load_cos_sin<true /*kInterleaved*/>(n_block);
    //                 rotary.template apply_K_interleaved<false>(sK(_, _, smem_pipe_read), gK_cur, tKpKs2g, tRrCos, tRrSin, tPrKPtr, n_block);
    //             } else {
    //                 auto [tRrCosCont, tRrSinCont] = rotary.template load_cos_sin<false /*kInterleaved*/>(n_block);
    //                 rotary.template apply_K_contiguous<false>(sK(_, _, smem_pipe_read), gK_cur, tKpKs2g, tRrCosCont, tRrSinCont, tPrKPtr, n_block, get<1>(params.shape_K));
    //             }
    //         }
    //     };

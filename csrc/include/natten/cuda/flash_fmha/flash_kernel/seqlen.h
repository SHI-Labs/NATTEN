...
struct SeqlenInfoQKNewK {
    ...
    SeqlenInfoQKNewK(int const bidb, int const seqlen_q_static, int const seqlen_k_static, int const shape_K_new_0)
                     // int const* const cu_seqlens_q=nullptr, int const* const cu_seqlens_k=nullptr, int const* const cu_seqlens_k_new=nullptr,
                     // int const* const seqused_q=nullptr, int const* const seqused_k=nullptr, int const* const ptr_leftpad_k=nullptr,
        ...
        , offset_k_new(0)
        , seqlen_q(seqlen_q_static)
        , seqlen_k_og(seqlen_k_static)
        , seqlen_k_new(0)
        , seqlen_k(seqlen_k_og)
        , seqlen_rotary(seqlen_k_og + leftpad_k)

        // NOTE (aditya): Keeping the original in case something blows up

...
void {kernel_name}(
    natten::cuda::flash::Flash_fwd_params params,
    cudaStream_t stream);
"""
...
void {kernel_name}(
    ...
    using Kernel = natten::cuda::flash::FlashForwardKernel<
      /* Arch= */ {cc},
      /* Element= */ {dtype},
      /* HeadDim= */ {GEMMShape[2]},
      /* kBlockM= */ {GEMMShape[0]},
    ...
    flash_fwd_kernel.run(params, stream);
}}
"""
...
def write_combined_source_file(path, filename, headers, kernels):
    ...
    source_head += ["#include <natten/cuda/flash_fmha/flash_fmha_forward.cuh>\n"]
...
class DTypeDispatcher:
    def __init__(self):
        self.dtypes = []
        self.name = "DISPATCH_FLASH_FMHA_FORWARD"
    ...
    def get_dispatcher(self):
        dispatcher_str = ""
        dispatcher_str += f"#define {self.name}(dtype, dim, cc, q_tile_size, kv_tile_size, ...) \\\n"
        dispatcher_str += "  [&] { \\\n"
        for i, dtype in enumerate(self.dtypes):
            ...
            dispatcher_str += " { \\\n"
            dispatcher_str += "    "
            dispatcher_str += f"  {self.name}_{dtype.short_name}(dim, cc, q_tile_size, kv_tile_size, __VA_ARGS__); \\\n"
            dispatcher_str += "    } \\\n"
        dispatcher_str += "    else { \\\n"
        dispatcher_str += '      std::cerr << "Flash FMHA forward kernel launch failed!" \\\n'
        dispatcher_str += (
            '                << "'
...
class HeadDimDispatcher:
    def __init__(self, dtype: DataType):
        self.dtype = dtype
        self.name = f"DISPATCH_FLASH_FMHA_FORWARD_{self.dtype.short_name}"
    ...
    def get_dispatcher(self):
        dispatcher_str = ""
        dispatcher_str += f"#define {self.name}(dim, cc, q_tile_size, kv_tile_size, ...) \\\n"
        dispatcher_str += "  [&] { \\\n"
        for i, dim in enumerate(self.dims):
            ...
            dispatcher_str += " { \\\n"
            dispatcher_str += "    "
            dispatcher_str += f"  {self.name}_headdim{dim}(cc, q_tile_size, kv_tile_size, __VA_ARGS__); \\\n"
            dispatcher_str += "    } \\\n"
        dispatcher_str += "    else { \\\n"
        dispatcher_str += '      std::cerr << "Flash FMHA forward kernel launch failed!" \\\n'
        dispatcher_str += (
...
class ArchDispatcher:
    def __init__(self, dtype: DataType, headdim: int):
        ...
        self.name = f"DISPATCH_FLASH_FMHA_FORWARD_{self.dtype.short_name}_headdim{self.headdim}"
    ...
    def get_dispatcher(self):
        dispatcher_str = ""
        dispatcher_str += f"#define {self.name}(cc, q_tile_size, kv_tile_size, ...) \\\n"
        dispatcher_str += "  [&] { \\\n"
        for i, cc in enumerate(self.ccs):
            ...
            dispatcher_str += " { \\\n"
            dispatcher_str += "    "
            dispatcher_str += f"  {self.name}_sm{cc}(q_tile_size, kv_tile_size, __VA_ARGS__); \\\n"
            dispatcher_str += "    } \\\n"
        dispatcher_str += "    else { \\\n"
        dispatcher_str += '      std::cerr << "Flash FMHA forward kernel launch failed!" \\\n'
        dispatcher_str += (
...
class ConfigDispatcher:
    def __init__(
        ...
        self.name = (
            f"DISPATCH_FLASH_FMHA_FORWARD_{self.dtype.short_name}_headdim{head_dim}_sm{cc}"
        )
...
def generate_flash_fmha_kernels(path, num_splits=2):
    ...
    path_to_sources = f"{path}/autogen/src/cuda/flash_fmha/"
    rel_header = "natten_autogen/cuda/flash_fmha/"
    ...
    cuda_headers = [
        ...
        "natten/natten.h",
        "natten/helpers.h",
        "natten/cuda/flash_fmha/flash_fmha_forward.cuh",
    ]
    write_header_file(
    ...
    write_header_file(
        arch_disp, path_arch, namespaces, cuda_headers + [rel_path_tile_size]
    )
    write_header_file(

...
#   NUM_GPUS: Number of GPUs to distribute tests across (default: 1)
#   WORKERS: Number of concurrent test workers (default: NATTEN_N_WORKERS or nproc/4)
...
# Validate NUM_GPUS is non-negative
if [ ${NUM_GPUS} -lt 0 ]; then
    echo "Error: NUM_GPUS must be >= 0 (got: ${NUM_GPUS})"
    exit 1
fi
...
# Handle WORKERS - check in order: argument, NATTEN_N_WORKERS, nproc/4
if [ -n "$2" ]; then
    WORKERS=$2

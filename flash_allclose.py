...
try:
    ...
    print("Using Flash Attention 3 for SM80")
    HAS_FLASH_ATTN = True
...
def test_fa2_allclose(ps: Problem):
    ...
    o_fa = flash_attn_func(q_fa, k_fa, v_fa)
    torch.cuda.synchronize()
...
@click.command()
@click.option('--b', default=1, type=int, help="Batch size")
@click.option('--h', default=24, type=int, help="Number of heads")
@click.option('--q', default=1019, type=int, help="Query length")
@click.option('--k', default=1024, type=int, help="Key length")
